...
class Grid
{
	...
	Cell * cell;														// We make it array of pointers not objects because
															// there are NO default constructor for class Cell

	static Card* cards[500];
...
public:
	...
	bool AddObjectToCell(GameObject * pNewObject);		 // Adds a GameObject to the Cell of its "position" data member
	                                                     ...
	                                                     // otherwise return false and don't add
	bool CheckCell(const CellPosition& pos);
	void SetNewCard( Card*);           
	Card* GetCard(const CellPosition& pos) const;
	void DeleteCard(const CellPosition&);

	void RemoveObjectFromCell(const CellPosition & pos); // Removes the GameObject of the Cell of the passed "position"
	void RemoveObjectFromCelloatr2();
	...
	void SetCurrPlayerNum(int playnum);
