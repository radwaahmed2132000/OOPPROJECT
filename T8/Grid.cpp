...
void Grid::DeleteCard(const CellPosition& pos)
{
  for(int i=0;i<num;i++)
	if(cards[i]->GetPosition().GetCellNum()==pos.GetCellNum())
		{
		 cards[i]=cards[num];
		 cards[num]=NULL;
		 num--;
...
{
	if (IsOverlapping(pNewObject) == true)
	{
...
void Grid::SetNewCard(Card*card)
{
...
Card* Grid::GetCard(const CellPosition& pos)const
{
		 for(int i=0;i<num;i++)
			 if(cards[i]->GetPosition().GetCellNum()==pos.GetCellNum())
				 return cards[i];
...
bool Grid::CheckCell(const CellPosition& pos)
{
	int i=0;
...
void Grid::RemoveObjectFromCelloatr2()
{
...
bool Grid::IsOverlapping(GameObject* newobj)
{
	...
	if (dynamic_cast<Snake*>(newobj))
	{
		for (int i = NumVerticalCells - 1; i >= 0; i--) // bottom up
		{
			for (int j = 0; j < NumHorizontalCells; j++) // left to right
			{
				if (newobj->IsOverlapping(CellList[i][j]->GetGameObject())!=true)
				{
				return  false;
				}
			}
...
void Grid::SetCurrPlayerNum(int playnum)
{
	if (playnum >= 0 && playnum < 4)
		currPlayerNumber = playnum;
	else
		currPlayerNumber = 0;
}

int Grid:: GetCurrPlayerNum()
{
