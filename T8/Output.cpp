...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_LADDER] = "images\\Menu_Ladder.jpg";	  //v
	MenuItemImages[ITM_ADD_SNAKE] = "images\\Menu_Snake.jpg";	//v
	MenuItemImages[ITM_ADD_CARD] = "images\\Menu_Card.jpg";	//v
	MenuItemImages[ITM_EXIT] = "images\\Menu_Exit.jpg";//v
	MenuItemImages[ITM_SWITCH_TO_PLAY_MODE] = "images\\Menu_SwitchToGame.jpg"; //v
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\Menu_Dice.jpg"; //v
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_InputDiceValue.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\Menu_NewGame.jpg";
	MenuItemImages[ITM_EXIT1] = "images\\Menu_Exit.jpg"; //v
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "images\\Menu_SwitchToGrid.jpg"; //v
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	int iX1 = cellStartX;
	int iY1 = cellStartY;
	int iX2 = cellStartX + UI.CellWidth - 1;
	int iY2 = cellStartY + UI.CellHeight - 1;
	pWind->DrawRectangle(iX1, iY1, iX2, iY2);   /////don't understand drawstyle
	...
	pWind->GetIntegerSize(w, h, cellNum);                        //w,h????????????????????????????????????????????
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (toCell.HCell() == fromCell.HCell() && toCell.VCell() < fromCell.VCell())//toCell.VCell() > fromCell.VCell() || toCell.VCell() < fromCell.VCell())
	{
		...
		for(int i=1; i <= toCell.VCell()-fromCell.VCell(); i++)
		{
			int y3=fromStartY + i*UI.CellHeight;
			pWind->DrawLine(x12,y3,x34,y3);
		}
		else
			for(int i=1; i<=abs(fromCell.VCell()-toCell.VCell()); i++)
			{
				int y3 = toStartY +i*UI.CellHeight;
				pWind->DrawLine(x12, y3, x34, y3);
			}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ( toCell.HCell() == fromCell.HCell ()&& toCell.VCell() > fromCell.VCell())
	{
